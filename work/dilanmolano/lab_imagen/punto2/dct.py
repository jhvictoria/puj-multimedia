...
ALPHA1 = math.sqrt(1 / 8);
ALPHA2 = math.sqrt(1 / 4);
...
def sumatory(a, b, F):
    ...
    for x in range(N):
        for y in range(M):
            ans += alpha(a) * (alpha(b) * F[x][y] * (math.cos(((x + .5) * a * math.pi) / float(N)) * math.cos(((y + .5) * b * math.pi) / float(M))));
    return ans;
...
def dct_encode(Fxy):
    ...
    for x in range(8):
        matrix.append([]);
        for y in range(8):
            matrix[u].append(int_round(sumatory(x, y, Fxy)));
...
def dct_decode(Tuv):
    ...
    for u in range(8):
        for v in range(8):
            matrix[u].append(int_round(sumatory(u, v, Tuv)));
...
def quantize(T, Q):
    ...
    for x in range(8):
        matrix.append([]);
        for y in range(8):
            matrix[x].append((int_round(T[x][y]) + 0.0) / Q[x][y]);
